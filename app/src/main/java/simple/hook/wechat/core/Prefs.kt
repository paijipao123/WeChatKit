 ...
 * 跨进程配置共享采用**双路径双写**策略：
 *
 * 1. 模块私有目录 `/data/user/0/simple.hook.wechat/shared_prefs/wechathook_prefs.xml`
 *    —— 设置页写入后立即 chmod 644（目录 711），使微信进程可以读取
 *    （Android SELinux 允许读取其他应用 world-readable 的 app_data_file）。
 * 2. 外部存储 `/storage/emulated/0/WeChatSimple/wechathook_prefs.xml`
...
object Prefs {
    ...
    private fun privateFile(): File =
        File("/data/user/0/simple.hook.wechat/shared_prefs/$FILE_NAME.xml")

    /** 跨进程共享路径（sdcard，辅助通道）。 */
    private fun sharedFile(): File =
        File("/storage/emulated/0/WeChatSimple/$FILE_NAME.xml")
    ...
    private fun tryLoadViaXSharedPreferencesInto(target: HashMap<String, Any?>): Boolean {
        return try {
            val prefs = de.robv.android.xposed.XSharedPreferences("simple.hook.wechat", FILE_NAME)
            try {
    ...
    private fun save() {
        runCatching {
            ...
            // 关键：让微信进程能读模块私有目录的配置
            // 整条目录链路放行: /data/user/0/simple.hook.wechat (711), shared_prefs (711)
            // 文件 644（全局可读）
            runCatching {
                val base = "/data/user/0/simple.hook.wechat"
                val cmds = arrayOf(
